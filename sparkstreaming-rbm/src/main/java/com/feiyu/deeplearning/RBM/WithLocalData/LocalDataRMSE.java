package com.feiyu.deeplearning.RBM.WithLocalData;
/**
...
import com.feiyu.deeplearning.RBM.RestrictedBoltzmannMachinesWithSoftmax;
import com.feiyu.utils.GlobalVariables;

import org.apache.log4j.Logger;

public class LocalDataRMSE {
  private static Logger log = Logger.getLogger(LocalDataRMSE.class.getName());
  int numMovies = 1682;
  ...
  public void start() throws IOException {
    ...
    bw.close();
    System.out.println("Saved rmse-by-epoch to "+GlobalVariables.LOG_DIR+RMSEfileName+".txt!!");
  }
      ...
      while ((line = br.readLine()) != null) {
        ...
        if (!user.equals(preUser)) {
          if (numUsers>0 && numUsers < numUserForTrain) {
            log.debug("train user " + ""+preUser+": "+ratedMoviesIndices.size());
        ...
        //log.debug("numUsers: "+numUsers+ ",preUser: "+preUser+",user: "+user+",movie: "+movie+",rating: "+rating + "-->"+ line);
        ratedMoviesIndices.add(new Tuple<Integer, Integer>(Integer.valueOf(movie),Integer.valueOf(rating)));
        preUser = user;
      }
      log.debug("Finished reading train.txt at epoch:"+epochs);
  ...
  public static void main(String[] argv) throws IOException {
    LocalDataRMSE getRE = new LocalDataRMSE();
    getRE.start();
  }
}
