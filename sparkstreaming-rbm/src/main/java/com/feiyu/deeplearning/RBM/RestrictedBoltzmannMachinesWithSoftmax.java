...
public class RestrictedBoltzmannMachinesWithSoftmax {
  ...
  public void trainRBMWeightMatrix(ArrayList<Tuple<Integer,Integer>> ratedMoviesIndices){
    ...
    log.debug("******Get recent 3D Weight Matrix..");
  ...
  public void predictUserPreference_VisibleToHiddenToVisible(ArrayList<Tuple<Integer,Integer>> ratedMoviesIndices,
    ...
    if (isForClient) {
      log.debug("********Get movies client might like..");
  ...
  private void initializeWeightMatrix() {
    ...
    log.debug("******Get the initialized 3D Weight Matrix..");
    this.printMatrix(this.numMovies+1, this.sizeHiddenUnits+1, this.sizeSoftmax, "weightMatrix");
  }	
  ...
  private void updateTheDataMatrix_oneUser(ArrayList<Tuple<Integer,Integer>> ratedMoviesIndices) {
    ...
    log.debug("******Get 3D Data Matrix -> One User..");
  ...
  private void getPhaMatrixOrNhaMatrix_oneUser(boolean isPositiveCD, boolean isForTrain) {
      ...
      for (int z=0; z<this.sizeSoftmax; z++) {
        ...
        for (int i=0; i<=this.numMovies; i++) {
          ...
          if (isPositiveCD) {
            ...
            //log.debug(0+" "+i+" "+z+" "+this.Md[0][i][z]+"--"+i+" "+y+" "+z+" "+this.Mw[i][y][z]+"--"+this.Md[0][i][z]*this.Mw[i][y][z]+" "+temp);
          } else {
    ...
    if (isPositiveCD) {
      log.debug("******Get 3D positive hidden associations matrix -> One User..");
      this.printMatrix(1, this.sizeHiddenUnits+1, this.sizeSoftmax, "phaMatrix");
    } else {
      log.debug("******Get 3D negative hidden associations matrix -> One User..");
  ...
  private void getPhpMatrixAndThePhsMatrixOrNhpMatrix_oneUser(boolean isPositiveCD) {
    ...
    for (int y=0; y<this.sizeHiddenUnits+1; y++) {
      ...
      for (int z=0; z<this.sizeSoftmax; z++) {
        //				log.debug(0+" "+y+" "+z+" "+Math.pow(Math.E, this.Mpha[0][y][z])+" "+currentSumOfSoftmax);
        if (isPositiveCD) {
    ...
    if (isPositiveCD) {
      log.debug("******Get 3D positive hidden probabilities matrix -> One User..");
      this.printMatrix(1, this.sizeHiddenUnits+1, this.sizeSoftmax, "phpMatrix");
      log.debug("******Get 3D positive hidden states matrix -> One User..");
      this.printMatrix(1, this.sizeHiddenUnits+1, this.sizeSoftmax, "phsMatrix");
    } else {
      log.debug("******Get 3D negative hidden probabilities matrix -> One User..");
  ...
  private void getPositiveOrNegativeWeightMatrix_ForNextStep(boolean isPositiveCD) {
    ...
    if (isPositiveCD) { 
      log.debug("******Get positive Weight Matrix --> next iteration..");
      this.printMatrix(this.numMovies+1, this.sizeHiddenUnits+1, this.sizeSoftmax, "weightMatrix_positive");
    } else {
      log.debug("******Get negative Weight Matrix --> next iteration..");
  ...
  private void getTheNvaMatrix_oneUser(boolean isForTrain) {
    ...
    log.debug("******Get 3D negtive visible activations Matrix -> One User..");
  ...
  private void getTheNvpMatrix_oneUser(boolean isForTrain) {
    ...
    log.debug("******Get 3D negative visible probabilities matrix -> One User..");
    this.printMatrix(1, this.numMovies+1, this.sizeSoftmax, "nvpMatrix");

    if (isForTrain) {
      this.setBiasUnitsToOnes_DataMatrix(false);
      log.debug("******Get 3D negative visible probabilities matrix (fixed bias units) -> One User..");
      this.printMatrix(1, this.numMovies+1, this.sizeSoftmax, "nvpMatrix");
    } else {
      log.debug("******Get predicted user preference..");
  ...
  private void getWeightMatrix_ForNextStep() {
    ...
    log.debug("******Get new Weight Matrix for the next step..");
    this.printMatrix(this.numMovies+1, this.sizeHiddenUnits+1, this.sizeSoftmax, "weightMatrix");
  }
  ...
  private void transposeDataMatrix(boolean isPositiveCD) {
    ...
    if (isPositiveCD) {
      log.debug("******Get transpose matrix of positive Data Matrix Md -> One User..");
      this.printMatrix(this.numMovies+1, 1, this.sizeSoftmax, "transposeDataMatrix_Positive");
    } else {
      log.debug("******Get transpose matrix of negative Data Matrix Mnvp -> One User..");
  ...
  private void transposeWeightMatrix(boolean isForTrain) {
    ...
    log.debug("******Get transpose matrix of Weight Matrix..");
    if (isForTrain) {
  ...
  /*
	private void getRMSE_oneUser() {
		...
		log.debug("rmse for current user: "+rmse);
	}
   */
  ...
  private void getRMSE_oneUser_onlyRatedMovies(ArrayList<Tuple<Integer,Integer>> ratedMoviesIndices) throws IOException {
    ...
    for (Tuple<Integer,Integer> ratedMovie : ratedMoviesIndices) {
      ...
      log.debug("curRealRating: "+ curRealRating +", curPredRating: "+curPredRating);
      rmse += Math.pow(curRealRating-curPredRating, 2);
    }
    this.RMSERBMModel += rmse;

    rmse = Math.sqrt(rmse/this.numMovies);
    log.debug("getRMSE_oneUser_onlyRatedMovies: rmse for current user: "+rmse);
