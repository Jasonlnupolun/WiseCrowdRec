...
public class StartMovieRecommendation {
  private static Logger log = Logger.getLogger(StartMovieRecommendation.class.getName());
  ...
  public void startMovieRec() throws IOException {
    ...
    while (itRatedMovie.hasNext()) {
      ...
      log.debug(">>>>>>>> ratedMovie: "+pairs.getKey() + " = " + pairs.getValue());
    ...
    for (int y=1; y<nMovies+1; y++) {
      ...
      log.debug("$$$$$ movie client might like: " + movieName);
    ...
    log.debug("d3Vertices:"+d3Vertices.toString());
    log.debug("d3Edges:"+d3Edges.toString());
    d3Data.put("nodes", d3Vertices);
    d3Data.put("links", d3Edges);
    String d3DataString = d3Data.toString();
    log.debug(" [x] RABBITMQ_QUEUE_NAME_SPARK Message Sent to queue buffer, movies client might like: "+ d3DataString);
  ...
  private void createFileForRMSE() throws IOException {
    File file = new File(GlobalVariables.LOG_DIR+"MovieRecommendation"
